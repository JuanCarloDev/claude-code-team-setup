...
**Batch variations** — When exploring multiple directions at once ("try different color palettes", "show me 5 variations"), use the `Task` tool to generate variations in parallel, just like Phase 2.
...
1. Copy the chosen concept as the starting point
2. Apply the user's feedback and save each new version with an incrementing number
3. Regenerate `logos/preview.html` after each iteration
...
- Keep SVG structure consistent across iterations (same group IDs)
- Use parallel agents for batch exploration (3+ variations), sequential writes for single tweaks
- **Check small-size legibility** — After generating iterations, include the favicon size check strip
...
1. Identify the final iteration SVG
...
The script produces: `logo-16.png` through `logo-2048.png`

5. If the export script fails, tell the user to install: `npm install -g @aspect-build/resvg`

## Phase 5: Repo Integration (optional)

If the user asks to commit the logo to a project repo or create a PR:

1. Check the repo for existing icon/logo files
2. Create a branch like `chore/new-logo`
3. Replace files — only replace files that already exist in the repo
4. Commit and PR
